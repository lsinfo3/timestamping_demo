...
struct ring * ring_init(int size) {
    ...
    ring->ringp = create_shared_memory((size+1) * sizeof(struct packetdata));
    memset(ring->ringp, 0, (size +1) * sizeof(struct packetdata));
    ring->head = 0;
    ring->tail = 0;
    ring->size = size+1;
    return ring;
...
int ring_push(struct ring * rb, struct packetdata * data){
    ...
    /* unsafe memcpy */
    memcpy(&rb->ringp[rb->head], data, sizeof(struct packetdata));
    rb->head = next;
    return 0;
}
/* pop from ringbuffer */
int ring_pop(struct ring * rb, struct packetdata * data) {
    ...
    memcpy(data, &rb->ringp[rb->tail], sizeof(struct packetdata));
    rb->tail = next;
    return 0;
}
